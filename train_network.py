    ...
    for idx, classname in enumerate(class_names):
        ...
        for idx2, infilename in enumerate(class_files[0:n_load]):          
            ...
            if (idx2 < n_train):
                ...
                if melgram.shape[3] < mel_dims[3]:
                    print("Skipping ",audio_path," with shape", melgram.shape)
                    continue
...
def build_model(X,Y,nb_classes):
    ...
    for layer in range(nb_layers-1):
        model.add(Conv2D(nb_filters, (kernel_size[0], kernel_size[1]), data_format='channels_first'))
        model.add(BatchNormalization(axis=1))
